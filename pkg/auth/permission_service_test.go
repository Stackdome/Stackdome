...
var _ = Describe("PermissionService.Check", func() {
	...
	Context("Identity validation", func() {
		It("should return Unauthenticated when no identity in context", func() {
			...
			err := env.permService.Check(context.Background(), teamABC, auth.ResourceStacks, "", auth.ActionList)
	...
	Context("API token scope enforcement", func() {
		...
		DescribeTable("scope checks for user with developer role under org 'OrgABC' in team 'TeamABC'",
			...
			Entry("matching scope", tokenPermCheck{
				scopes: []string{"stacks:*"}, domain: teamABC, resource: auth.ResourceStacks, action: auth.ActionList, shouldAllow: true,
			}),
			Entry("scope doesnt cover resource", tokenPermCheck{
				scopes: []string{"secrets:*"}, domain: teamABC, resource: auth.ResourceStacks, action: auth.ActionList, shouldAllow: false,
			}),
			Entry("full access scope", tokenPermCheck{
				scopes: []string{"*:*"}, domain: teamABC, resource: auth.ResourceStacks, action: auth.ActionList, shouldAllow: true,
			}),
			Entry("parent scope addons:* - read", tokenPermCheck{
				scopes: []string{"addons:*"}, domain: teamABC, resource: auth.ResourceAddonsPostgres, resourceID: "addon-1", action: auth.ActionRead, shouldAllow: true,
			}),
			Entry("parent scope addons:* - create", tokenPermCheck{
				scopes: []string{"addons:*"}, domain: teamABC, resource: auth.ResourceAddonsPostgres, resourceID: "addon-1", action: auth.ActionCreate, shouldAllow: true,
			}),
			Entry("specific action mismatch", tokenPermCheck{
				scopes: []string{"stacks:read"}, domain: teamABC, resource: auth.ResourceStacks, resourceID: "s-1", action: auth.ActionWrite, shouldAllow: false,
			}),
			Entry("resourceID restriction matching", tokenPermCheck{
				scopes: []string{"stacks:*"}, resourceIDs: []string{"s-1"}, domain: teamABC, resource: auth.ResourceStacks, resourceID: "s-1", action: auth.ActionRead, shouldAllow: true,
			}),
			Entry("resourceID restriction matching but scope action mismatch", tokenPermCheck{
				scopes: []string{"stacks:read"}, resourceIDs: []string{"s-1"}, domain: teamABC, resource: auth.ResourceStacks, resourceID: "s-1", action: auth.ActionWrite, shouldAllow: false,
			}),
			Entry("resourceID restriction not matching", tokenPermCheck{
				scopes: []string{"stacks:*"}, resourceIDs: []string{"s-1"}, domain: teamABC, resource: auth.ResourceStacks, resourceID: "s-2", action: auth.ActionRead, shouldAllow: false,
			}),
			Entry("empty resourceIDs no restriction", tokenPermCheck{
				scopes: []string{"stacks:*"}, domain: teamABC, resource: auth.ResourceStacks, resourceID: "any-id", action: auth.ActionRead, shouldAllow: true,
			}),
		)
	})
	...
	Context("Developer role", func() {
		...
		DescribeTable("team-scoped resources",
			...
			Entry("list stacks in own team", permCheck{
				domain: teamABC, resource: auth.ResourceStacks, action: auth.ActionList, shouldAllow: true,
			}),
			Entry("create stack in own team", permCheck{
				domain: teamABC, resource: auth.ResourceStacks, action: auth.ActionCreate, shouldAllow: true,
			}),
			Entry("read specific stack", permCheck{
				domain: teamABC, resource: auth.ResourceStacks, resourceID: "stack-1", action: auth.ActionRead, shouldAllow: true,
			}),
			Entry("write specific stack", permCheck{
				domain: teamABC, resource: auth.ResourceStacks, resourceID: "stack-1", action: auth.ActionWrite, shouldAllow: true,
			}),
			Entry("delete specific stack", permCheck{
				domain: teamABC, resource: auth.ResourceStacks, resourceID: "stack-1", action: auth.ActionDelete, shouldAllow: true,
			}),
			Entry("list secrets", permCheck{
				domain: teamABC, resource: auth.ResourceSecrets, action: auth.ActionList, shouldAllow: true,
			}),
			Entry("create secret", permCheck{
				domain: teamABC, resource: auth.ResourceSecrets, action: auth.ActionCreate, shouldAllow: true,
			}),
			Entry("read volume", permCheck{
				domain: teamABC, resource: auth.ResourceVolumes, resourceID: "vol-1", action: auth.ActionRead, shouldAllow: true,
			}),
			Entry("write volume", permCheck{
				domain: teamABC, resource: auth.ResourceVolumes, resourceID: "vol-1", action: auth.ActionWrite, shouldAllow: true,
			}),
			Entry("delete volume", permCheck{
				domain: teamABC, resource: auth.ResourceVolumes, resourceID: "vol-1", action: auth.ActionDelete, shouldAllow: true,
			}),
			Entry("read addon", permCheck{
				domain: teamABC, resource: auth.ResourceAddonsPostgres, resourceID: "addon-1", action: auth.ActionRead, shouldAllow: true,
			}),
			Entry("write addon", permCheck{
				domain: teamABC, resource: auth.ResourceAddonsPostgres, resourceID: "addon-1", action: auth.ActionWrite, shouldAllow: true,
			}),
			Entry("list object stores", permCheck{
				domain: teamABC, resource: auth.ResourceObjectStores, action: auth.ActionList, shouldAllow: true,
			}),
			Entry("delete object store", permCheck{
				domain: teamABC, resource: auth.ResourceObjectStores, resourceID: "os-1", action: auth.ActionDelete, shouldAllow: true,
			}),
			Entry("create workspace user", permCheck{
				domain: teamABC, resource: auth.ResourceWorkspaceUsers, action: auth.ActionCreate, shouldAllow: true,
			}),
			Entry("write workspace user", permCheck{
				domain: teamABC, resource: auth.ResourceWorkspaceUsers, resourceID: "wu-1", action: auth.ActionWrite, shouldAllow: true,
			}),
			...
			Entry("cannot access other teams stacks", permCheck{
				domain: teamXYZ, resource: auth.ResourceStacks, action: auth.ActionList, shouldAllow: false,
			}),
			Entry("cannot read other teams secret", permCheck{
				domain: teamXYZ, resource: auth.ResourceSecrets, resourceID: "sec-1", action: auth.ActionRead, shouldAllow: false,
			}),
		)
	})
	...
	Context("Viewer role", func() {
		...
		DescribeTable("read-only access",
			...
			Entry("read secret", permCheck{
				domain: teamABC, resource: auth.ResourceSecrets, resourceID: "sec-1", action: auth.ActionRead, shouldAllow: true,
			}),
			Entry("list volumes", permCheck{
				domain: teamABC, resource: auth.ResourceVolumes, action: auth.ActionList, shouldAllow: true,
			}),
			Entry("read volume", permCheck{
				domain: teamABC, resource: auth.ResourceVolumes, resourceID: "vol-1", action: auth.ActionRead, shouldAllow: true,
			}),
			...
			Entry("read object store", permCheck{
				domain: teamABC, resource: auth.ResourceObjectStores, resourceID: "os-1", action: auth.ActionRead, shouldAllow: true,
			}),

			// Write access — denied
			Entry("cannot create stack", permCheck{
				domain: teamABC, resource: auth.ResourceStacks, action: auth.ActionCreate, shouldAllow: false,
			}),
			Entry("cannot write stack", permCheck{
				domain: teamABC, resource: auth.ResourceStacks, resourceID: "stack-1", action: auth.ActionWrite, shouldAllow: false,
			}),
			Entry("cannot delete stack", permCheck{
				domain: teamABC, resource: auth.ResourceStacks, resourceID: "stack-1", action: auth.ActionDelete, shouldAllow: false,
			}),
			Entry("cannot create secret", permCheck{
				domain: teamABC, resource: auth.ResourceSecrets, action: auth.ActionCreate, shouldAllow: false,
			}),
			Entry("cannot delete secret", permCheck{
				domain: teamABC, resource: auth.ResourceSecrets, resourceID: "sec-1", action: auth.ActionDelete, shouldAllow: false,
			}),
			Entry("cannot delete addon", permCheck{
				domain: teamABC, resource: auth.ResourceAddonsPostgres, resourceID: "addon-1", action: auth.ActionDelete, shouldAllow: false,
			}),
	...
	Context("OrgMember role (no team membership)", func() {
		...
		DescribeTable("org-scoped access only",
			...
			Entry("list teams", permCheck{
				domain: orgABC, resource: auth.ResourceTeams, action: auth.ActionList, shouldAllow: true,
			}),
			Entry("read team", permCheck{
				domain: orgABC, resource: auth.ResourceTeams, resourceID: "team-1", action: auth.ActionRead, shouldAllow: true,
			}),
			...
			Entry("cannot list stacks", permCheck{
				domain: teamABC, resource: auth.ResourceStacks, action: auth.ActionList, shouldAllow: false,
			}),
			Entry("cannot list secrets", permCheck{
				domain: teamXYZ, resource: auth.ResourceSecrets, action: auth.ActionList, shouldAllow: false,
			}),
		)
	})
	...
	Context("OrgAdmin fallback", func() {
		...
		DescribeTable("access via OrgAdmin",
			...
			Entry("delete secret in team", permCheck{
				domain: teamABC, resource: auth.ResourceSecrets, resourceID: "sec-1", action: auth.ActionDelete, shouldAllow: true,
			}),
			Entry("create addon in team", permCheck{
				domain: teamABC, resource: auth.ResourceAddonsPostgres, action: auth.ActionCreate, shouldAllow: true,
			}),
	...
	Context("Cross-team isolation", func() {
		It("should isolate users to their own teams", func() {
			...
			Expect(env.permService.Check(ctx1, teamABC, auth.ResourceStacks, "", auth.ActionList)).To(BeNil())
			Expect(env.permService.Check(ctx1, teamXYZ, auth.ResourceStacks, "", auth.ActionList)).ToNot(BeNil())
			Expect(env.permService.Check(ctx2, teamXYZ, auth.ResourceStacks, "", auth.ActionList)).To(BeNil())
			Expect(env.permService.Check(ctx2, teamABC, auth.ResourceStacks, "", auth.ActionList)).ToNot(BeNil())
		})
	})
	...
	Context("Multi-role user", func() {
		...
		DescribeTable("permissions depend on team role",
			...
			Entry("write in Developer team", permCheck{
				domain: teamABC, resource: auth.ResourceStacks, resourceID: "s-1", action: auth.ActionWrite, shouldAllow: true,
			}),
			Entry("delete in Developer team", permCheck{
				domain: teamABC, resource: auth.ResourceStacks, resourceID: "s-1", action: auth.ActionDelete, shouldAllow: true,
			}),
			Entry("read in Viewer team", permCheck{
				domain: teamXYZ, resource: auth.ResourceStacks, resourceID: "s-1", action: auth.ActionRead, shouldAllow: true,
			}),
			Entry("cannot write in Viewer team", permCheck{
				domain: teamXYZ, resource: auth.ResourceStacks, resourceID: "s-1", action: auth.ActionWrite, shouldAllow: false,
			}),
			Entry("cannot delete in Viewer team", permCheck{
				domain: teamXYZ, resource: auth.ResourceStacks, resourceID: "s-1", action: auth.ActionDelete, shouldAllow: false,
			}),
			Entry("cannot create in Viewer team", permCheck{
				domain: teamXYZ, resource: auth.ResourceStacks, action: auth.ActionCreate, shouldAllow: false,
			}),
		)
	})
		...
		Context("Viewer with token", func() {
			...
			DescribeTable("token cannot expand Viewer permissions",
				...
				Entry("full token viewer can read", tokenPermCheck{
					scopes: []string{"*:*"}, domain: teamABC, resource: auth.ResourceStacks, resourceID: "s-1", action: auth.ActionRead, shouldAllow: true,
				}),
				Entry("full token viewer cannot write", tokenPermCheck{
					scopes: []string{"*:*"}, domain: teamABC, resource: auth.ResourceStacks, resourceID: "s-1", action: auth.ActionWrite, shouldAllow: false,
				}),
				Entry("full token viewer cannot delete", tokenPermCheck{
					scopes: []string{"*:*"}, domain: teamABC, resource: auth.ResourceSecrets, resourceID: "s-1", action: auth.ActionDelete, shouldAllow: false,
				}),
				Entry("full token viewer cannot create", tokenPermCheck{
					scopes: []string{"*:*"}, domain: teamABC, resource: auth.ResourceStacks, action: auth.ActionCreate, shouldAllow: false,
				}),
				Entry("stacks token viewer reads stack", tokenPermCheck{
					scopes: []string{"stacks:*"}, domain: teamABC, resource: auth.ResourceStacks, resourceID: "s-1", action: auth.ActionRead, shouldAllow: true,
				}),
				Entry("stacks token blocks secret read", tokenPermCheck{
					scopes: []string{"stacks:*"}, domain: teamABC, resource: auth.ResourceSecrets, resourceID: "s-1", action: auth.ActionRead, shouldAllow: false,
				}),
				Entry("read token viewer reads", tokenPermCheck{
					scopes: []string{"stacks:read"}, domain: teamABC, resource: auth.ResourceStacks, resourceID: "s-1", action: auth.ActionRead, shouldAllow: true,
				}),
				Entry("list token viewer lists", tokenPermCheck{
					scopes: []string{"stacks:list"}, domain: teamABC, resource: auth.ResourceStacks, action: auth.ActionList, shouldAllow: true,
				}),
		...
		Context("Developer with restricted token", func() {
			...
			DescribeTable("token restricts Developer permissions",
				...
				Entry("secrets token blocks developer stacks", tokenPermCheck{
					scopes: []string{"secrets:*"}, domain: teamABC, resource: auth.ResourceStacks, resourceID: "s-1", action: auth.ActionRead, shouldAllow: false,
				}),
				Entry("stacks token allows developer stacks", tokenPermCheck{
					scopes: []string{"stacks:*"}, domain: teamABC, resource: auth.ResourceStacks, resourceID: "s-1", action: auth.ActionWrite, shouldAllow: true,
				}),
			)
		})
