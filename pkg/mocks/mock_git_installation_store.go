// Code generated by MockGen. DO NOT EDIT.
// Source: git_installation.go
//
// Generated by this command:
//
//	mockgen -source=git_installation.go -destination=../mocks/mock_git_installation_store.go -package=mocks
//

// Package mocks is a generated GoMock package.
package mocks

import (
	context "context"
	reflect "reflect"

	errors "github.com/Stackdome/stackdome/pkg/errors"
	models "github.com/Stackdome/stackdome/pkg/models"
	gomock "go.uber.org/mock/gomock"
)

// MockGitInstallationStore is a mock of GitInstallationStore interface.
type MockGitInstallationStore struct {
	ctrl     *gomock.Controller
	recorder *MockGitInstallationStoreMockRecorder
	isgomock struct{}
}

// MockGitInstallationStoreMockRecorder is the mock recorder for MockGitInstallationStore.
type MockGitInstallationStoreMockRecorder struct {
	mock *MockGitInstallationStore
}

// NewMockGitInstallationStore creates a new mock instance.
func NewMockGitInstallationStore(ctrl *gomock.Controller) *MockGitInstallationStore {
	mock := &MockGitInstallationStore{ctrl: ctrl}
	mock.recorder = &MockGitInstallationStoreMockRecorder{mock}
	return mock
}

// EXPECT returns an object that allows the caller to indicate expected use.
func (m *MockGitInstallationStore) EXPECT() *MockGitInstallationStoreMockRecorder {
	return m.recorder
}

// DeleteByInstallationID mocks base method.
func (m *MockGitInstallationStore) DeleteByInstallationID(ctx context.Context, integrationID string, installationID int64) *errors.ServiceError {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "DeleteByInstallationID", ctx, integrationID, installationID)
	ret0, _ := ret[0].(*errors.ServiceError)
	return ret0
}

// DeleteByInstallationID indicates an expected call of DeleteByInstallationID.
func (mr *MockGitInstallationStoreMockRecorder) DeleteByInstallationID(ctx, integrationID, installationID any) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	return mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "DeleteByInstallationID", reflect.TypeOf((*MockGitInstallationStore)(nil).DeleteByInstallationID), ctx, integrationID, installationID)
}

// DeleteByIntegrationID mocks base method.
func (m *MockGitInstallationStore) DeleteByIntegrationID(ctx context.Context, integrationID string) *errors.ServiceError {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "DeleteByIntegrationID", ctx, integrationID)
	ret0, _ := ret[0].(*errors.ServiceError)
	return ret0
}

// DeleteByIntegrationID indicates an expected call of DeleteByIntegrationID.
func (mr *MockGitInstallationStoreMockRecorder) DeleteByIntegrationID(ctx, integrationID any) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	return mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "DeleteByIntegrationID", reflect.TypeOf((*MockGitInstallationStore)(nil).DeleteByIntegrationID), ctx, integrationID)
}

// GetByIntegrationAndAccount mocks base method.
func (m *MockGitInstallationStore) GetByIntegrationAndAccount(ctx context.Context, integrationID, accountLogin string) (*models.GitInstallation, *errors.ServiceError) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "GetByIntegrationAndAccount", ctx, integrationID, accountLogin)
	ret0, _ := ret[0].(*models.GitInstallation)
	ret1, _ := ret[1].(*errors.ServiceError)
	return ret0, ret1
}

// GetByIntegrationAndAccount indicates an expected call of GetByIntegrationAndAccount.
func (mr *MockGitInstallationStoreMockRecorder) GetByIntegrationAndAccount(ctx, integrationID, accountLogin any) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	return mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "GetByIntegrationAndAccount", reflect.TypeOf((*MockGitInstallationStore)(nil).GetByIntegrationAndAccount), ctx, integrationID, accountLogin)
}

// ListByIntegrationID mocks base method.
func (m *MockGitInstallationStore) ListByIntegrationID(ctx context.Context, integrationID string) ([]*models.GitInstallation, *errors.ServiceError) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "ListByIntegrationID", ctx, integrationID)
	ret0, _ := ret[0].([]*models.GitInstallation)
	ret1, _ := ret[1].(*errors.ServiceError)
	return ret0, ret1
}

// ListByIntegrationID indicates an expected call of ListByIntegrationID.
func (mr *MockGitInstallationStoreMockRecorder) ListByIntegrationID(ctx, integrationID any) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	return mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "ListByIntegrationID", reflect.TypeOf((*MockGitInstallationStore)(nil).ListByIntegrationID), ctx, integrationID)
}

// Upsert mocks base method.
func (m *MockGitInstallationStore) Upsert(ctx context.Context, installation *models.GitInstallation) (*models.GitInstallation, *errors.ServiceError) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "Upsert", ctx, installation)
	ret0, _ := ret[0].(*models.GitInstallation)
	ret1, _ := ret[1].(*errors.ServiceError)
	return ret0, ret1
}

// Upsert indicates an expected call of Upsert.
func (mr *MockGitInstallationStoreMockRecorder) Upsert(ctx, installation any) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	return mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "Upsert", reflect.TypeOf((*MockGitInstallationStore)(nil).Upsert), ctx, installation)
}
