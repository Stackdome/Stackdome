...
func (v *volumeStore) ListByProjectID(ctx context.Context, projectID string) ([]*models.Volume, *errors.ServiceError) {
	var volumes []*models.Volume
	if err := v.sessionFactory.New(ctx).
		Where("project_id = ?", projectID).
		Order("created_at DESC").
		Find(&volumes).Error; err != nil {
		return nil, errors.GeneralError("failed to list volumes by project: %s", err.Error())
	}
	return volumes, nil
}
