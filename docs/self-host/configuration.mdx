...
| `GITHUB_REDIRECT_URI` | No | `<SERVER_EXTERNAL_URL>/auth/github/callback` | Where GitHub sends the browser after the user authorizes. Set it explicitly only when that derived browser callback is not correct for your deployment. |
