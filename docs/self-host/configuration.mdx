...
The API server takes all of its configuration from environment variables. If you installed Stackdome with the installer, the secrets it needs were already generated and wired in for you — see [Install](/self-host/install). This page is the reference for changing those values afterwards, or for running the API server yourself.
...
| `GITHUB_REDIRECT_URI` | No | derived from `SERVER_EXTERNAL_URL` | Where GitHub sends the browser after the user authorizes |
...
<Warning>
Set this one explicitly. When `GITHUB_REDIRECT_URI` is unset, the server derives it from `SERVER_EXTERNAL_URL` as `<SERVER_EXTERNAL_URL>/api/v1/auth/github/callback` — that points at the API endpoint rather than the UI page that completes the sign-in, so the derived default is not the value you want in a browser flow.
</Warning>

## Platform cluster

These seed a cluster and its base domain into the database the first time the server boots, so an install can come up with a cluster already registered instead of you adding one by hand. Leave them unset if you would rather register clusters through the UI.
...
| Variable | Required | Default | Purpose |
| --- | --- | --- | --- |
| `STACKDOME_ENV` | No | `DEVELOPMENT` | Which runtime environment the server initializes as. `DEVELOPMENT` is the only value the server accepts. |

<Warning>
Leave `STACKDOME_ENV` unset, or set it to `DEVELOPMENT`. Any other value — including `PRODUCTION` — makes the server panic at startup with `env: <value> not defined`, before it reaches configuration validation. A self-hosted production install runs with this variable unset.
</Warning>

## What the installer already set

The installer generates the three secret values on first run, keeps them in a Kubernetes secret named `stackdome-bootstrap-secrets` in the `stackdome-control-plane` namespace, and reuses them on every later run so a re-install does not invalidate your data:
...
It also sets `DB_HOST`, `DB_PORT`, `DB_NAME`, `DB_USERNAME`, and `LOG_LEVEL=info` to point the server at the PostgreSQL instance it deployed alongside it. Everything else on this page is unset and falls back to the defaults above.
