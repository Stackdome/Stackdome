...
## Canvas is now the default (flag flipped to opt-out)
- Per the user ("prefer canvas first; old form can be hidden permanently, remove later"), `isCanvasEnabled()` now **defaults to true**. The legacy accordion form is reachable only via an explicit **opt-out**: `VITE_STACK_CANVAS=false` (build) or `localStorage.stackCanvas="0"` (per-browser). The old form + `StackResourcesForm`/detail path are **left in place for now** (removal is a later cleanup).
- `feature-flags.test.ts` updated: default true (canvas-first), `"0"` → legacy form, any other value stays on.
- Verified (Playwright): with **no flag set**, `/stacks/:id` renders the full-bleed canvas (4 nodes, no `max-w-6xl`); with `stackCanvas="0"` it renders the old STACK RESOURCES form in the constrained layout. tsc/eslint clean, 515 tests pass.

## Wizard → canvas directly (user feedback)
- User: *"when I open editor from wizard it still shows form"* + *"just from the dashboard wizard move to canvas directly."* The wizard's "Open editor" went to `/stacks/create` (a form that also **names** the stack). The canvas is the show/edit page for an existing stack, so reaching it requires creating first.
- **Composer now creates + opens the canvas** (`block-composer.tsx`). Added a **Stack name** field to the composer's right panel. Continue → `createAndOpen`: build `FormStackData` from the composed stack, `FormStackSchema.safeParse`; if named + valid + org/team resolve → `createStack` → navigate `/stacks/:id` (canvas) with `state.linkedAddonIds`. Otherwise (no name, unconfigured resource, missing org/team, or API error) → **fall back** to the `/stacks/create` form (which now also lands on the canvas after save). Footer hint → "Open in the canvas editor" / "Creating your stack…".
- **Selected managed addons carried into the canvas.** `detail/index.tsx` reads `location.state.linkedAddonIds`; once the created stack is fetched it opens an edit session **pre-linked** with them (so they render as addon nodes and can be bound + saved in the canvas), then clears the nav state. (Unbound addons aren't persisted as connections on create, so this re-links them client-side — the canvas drawer's Environment tab binds them, and Save persists.)
- **`create/index.tsx` success → canvas.** After `createStack`, navigate to `/stacks/:id` (was the `/stacks` list), carrying `linkedAddonIds`.
- **Verified (Playwright):** New Stack → Build from blocks → add Redis → name "canvas-wizard-check" → Continue → **stack created + landed on the canvas** (redis node, PENDING, no form). The existing `block-composer.test.tsx` stays green (an unnamed composed stack hits the fallback → `/stacks/create`). tsc/eslint clean, 515 tests pass.
- **Note:** verification created one real, **undeployed** stack (`canvas-wizard-check`, id `6c8c183d-…`); no Deploy was triggered, but a stack delete UI isn't wired yet, so it will linger until removed out-of-band.
- **Deferred:** the canvas has no "link a managed addon" affordance of its own yet (only bind an already-linked one) — linking still starts in the wizard/form. A canvas-native addon picker is a follow-up.
