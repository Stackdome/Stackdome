...
description: "Take a git repository from zero to a live URL on Stackdome."
---
...
- **A Stackdome install you can sign in to.** If you do not have one yet, the [quickstart](/quickstart) takes a bare Linux server to a working dashboard.
...
<Note>
You are never asked which cluster to deploy to. A stack runs on the cluster registered for your organization, so the create flow has no cluster picker — there is nothing to choose.
</Note>
...
That is the whole source configuration. The **Advanced** row underneath holds the `build & push` settings — the Dockerfile path, the build context, and where to push the finished image — and their defaults are `Dockerfile`, `.`, and the image registry inside your cluster. You only open it if your Dockerfile is not at the repository root.
