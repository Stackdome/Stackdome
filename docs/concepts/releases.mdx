---
title: "Releases"
description: "How a deploy becomes a release, how status is reported, and how to roll back."
---

A stack is what you edit. A **release** is what you shipped. Every deploy creates one, and the **Deployments** tab of a stack is the list of them, newest first.

## What a release is

A release is one deploy of one stack, numbered per stack — `#1`, `#2`, `#3` — and it carries a complete copy of the stack as it was at the moment you pressed deploy: every resource, its source, its ports, its environment, every volume and every mount, and the connections between nodes on the canvas.
...
You can also stop a deploy: the release's **⋮** menu offers **Cancel release** while it is `Pending`. Once it is `InProgress` the rollout has already been handed to the cluster, so cancelling is no longer offered — deploy again, or roll back.
