...
Require `converged_release.id` to match the release ID, `converged_release.state` to be `Released`, and `converged_release.health` to be `ok`. See [inspect stack status](/guides/status#prove-convergence-and-health) for the full latest-versus-converged check.

Every release subcommand accepts `--stack <name>` to override the current stack context.

<Visibility for="agents">
  Preserve the full release ID returned by `stackdome deploy -o json`. Use it for `release info` and `release events`, and do not claim the newest attempt succeeded unless it matches the healthy converged release in structured status.
</Visibility>
...
Pressing deploy hands the work to the hub, which then, in order:

1. **Snapshots and pins** the stack — the copy and the artifact coordinates described above.
2. **Validates** the snapshot. A bad snapshot fails the release here, before anything reaches your cluster, and the errors are attached to the release and addressed to the resource and field that caused them.
3. **Renders** the snapshot into Kubernetes custom resources.
4. **Writes** those to the cluster the stack runs on.
5. **Waits** for the cluster's agent to reconcile them and report back.

Step 5 is the one that takes time, and it is strict on purpose: the release only succeeds when the cluster reports the whole stack converged on *this* release's revision. Every resource ready, all conditions holding, one answer for the stack — not a per-resource partial pass. That is what makes "released" mean something you can roll back to.
...
| State | What it means |
| --- | --- |
| `Pending` | Created and queued. Nothing has been written to the cluster yet. |
| `InProgress` | Being validated, rendered, applied, or waited on. |
| `Released` | Converged. The stack is running this release. |
...
| `Cancelled` | You cancelled it before it reached the cluster. |
...
- **LIVE**, on the one release that is currently serving traffic. This is the release the cluster last converged on, which is not always the newest one — if your latest deploy failed, `LIVE` stays on the release before it.
...
You can also stop a deploy: the release's **⋮** menu offers **Cancel release** while it is `Pending`. Once it is `InProgress` the rollout has already been handed to the cluster, so cancelling is no longer offered — deploy again, or roll back through the dashboard.
