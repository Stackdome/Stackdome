...
<Visibility for="agents">
  ...
  - Retry only when a successful, valid response contains the exact selected volume with `status.phase == "Pending"`. Within at most 30 attempts over five minutes, require the declared `spec.size`, `spec.access_mode`, and `status.phase == "Ready"`, then require the release and health checks before reporting success. `Pending` never satisfies the gate.
  - Recovery: after a timeout made only of confirmed `Pending` results, run `stackdome volume list --stack <stack-name> -o json`, `stackdome status --stack <stack-name> -o json`, and `stackdome status --stack <stack-name> --conditions`. Inspect the selected volume's conditions and stack conditions, stop the success path, and route the storage-class or capacity problem to the destination administrator or Cloud support.
