...
Require the deployment's release state to be `Released`, then apply the [status convergence and health checks](/guides/status#prove-convergence-and-health). Do not treat an older successful build as proof that the newest release used it.
