...
- A registry reachable from both the Stackdome hub, which probes the image before release, and the target cluster, which performs the pull.
...
<CardGroup cols={2}>
  ...
  <Card title="Deploy from git" icon="code-branch" href="/guides/deploy-from-git">
    Let the target cluster build a repository and pin its resolved revision per release.
  </Card>
