...
To prove that the **newest attempt** is the healthy serving release, also require:

- `latest_release.id` equals `converged_release.id`.
...
<Visibility for="agents">
  Do not claim that the latest deployment succeeded from `converged_release.health == "ok"` alone. An older release can still be healthy after the latest attempt fails. Match the two release IDs as well.
</Visibility>
...
This is a stream of JSON values, not one JSON array. Parse each value independently and stop only after the latest and converged release IDs match, both states are `Released`, and converged health is `ok`.
