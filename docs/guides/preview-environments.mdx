...
- An administrator who can enable the repository in the dashboard or API. The current public CLI has no preview-configuration, sync, list, or delete commands.

## Agent prompt

```text
Prepare this repository for per-pull-request preview environments. Inspect the application, create or update the committed stackfile.yaml with its resources and public ports, validate it with the current Stackdome CLI, and identify the exact dashboard-only repository configuration an administrator must enable. After a preview is created, verify its Ready state and public URL. Do not invent preview CLI commands or claim that validation alone enabled the automation.
```
...
<Steps>
  ...
  <Step title="Hand off dashboard-only setup">
    Ask an administrator to use **Platform → Previews → Enable repository** and select the connected repository, base branch, Stackfile path, active-preview cap, and configuration-level environment values. The CLI cannot perform this step.
  </Step>
...
<Visibility for="agents">
  ...
  - Configuration, manual creation, sync, and deletion are dashboard/API-only. Ask for administrator action when those mutations are required.
</Visibility>
...
- A `build` needs `repo`, plus at most one of `branch`, `tag` or `commit`. Setting none of the three is fine — the repository's default branch is used.
...
Teardown deletes the environment's stack and everything under it, waits for the cluster to finish removing it, then removes the environment record and marks the sticky comment as deleted. Nothing expires on a timer, so an environment attached to a long-lived pull request keeps running — and keeps consuming cluster capacity — until that pull request closes.
...
**There is no build-concurrency limit in Stackdome.** Nothing queues or throttles simultaneous builds: if ten pull requests are opened at once, ten sets of builds start at once and compete for the same node CPU. On a single-node cluster that is enough to saturate the node and push builds into deploy timeouts.

The only limit is **Max active previews**, per configuration, defaulting to `10`. It caps how many environments can exist at once — not how many build at once. When a configuration is at its cap, a new pull request creates nothing: the delivery is logged as skipped and no comment is posted, so the pull request looks like previews are simply not enabled. Set the cap to what your cluster can actually hold.
</Warning>
