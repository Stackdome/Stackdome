...
const ORG = "org1";
const TEAM = "default";
const BASE = `/organizations/${ORG}/teams/${TEAM}/stack-preview-configs`;
...
describe("preview-configs api", () => {
  it("lists configs with pagination", async () => {
    ...
    const out = await listPreviewConfigs(ORG, TEAM, 2, 50);
  ...
  it("gets one config", async () => {
    ...
    const out = await getPreviewConfig(ORG, TEAM, "c1");
  ...
  it("creates a config", async () => {
    ...
    await createPreviewConfig(ORG, TEAM, input);
  ...
  it("updates a config", async () => {
    ...
    await updatePreviewConfig(ORG, TEAM, "c1", { stackfile_path: "deploy/stackfile.yaml" });
    expect(api.put).toHaveBeenCalledWith(`${BASE}/c1`, { stackfile_path: "deploy/stackfile.yaml" });
  });

  it("deletes a config", async () => {
    (api.delete as ReturnType<typeof vi.fn>).mockResolvedValue({});
    await deletePreviewConfig(ORG, TEAM, "c1");
