...
export function OutcomesTable({ outcomes }: OutcomesTableProps) {
  ...
  return (
    <div>
      <div className="grid grid-cols-[1fr_90px_1fr] gap-2 pb-1.5 font-mono text-[10px] uppercase tracking-wide text-fg-muted">
        <span>Resource</span><span>Phase</span><span>Message</span>
      </div>
      {rows.map(([name, o]) => {
        const tone = phaseTone(o.phase ?? "");
        return (
          <div key={name} className="grid grid-cols-[1fr_90px_1fr] items-center gap-2 border-t border-border py-2">
