...
var _ = Describe("Release E2E", Ordered, func() {
	...
	Context("Release Lifecycle", func() {
		It("should deploy via release and reach Released state", func() {
			...
			released := shared.WaitForReleaseReleased(client, orgID, projectName, stackID, release.GetId(), 5*time.Minute)
			...
			shared.WaitForStackReady(client, orgID, projectName, stackID, 2*time.Minute)
			...
			list := shared.ListReleases(client, orgID, projectName, stackID)
			Expect(list.GetItems()).To(HaveLen(1))

			By("Fetching the release event timeline")
			events := shared.ListReleaseEvents(client, orgID, projectName, stackID, release.GetId()).GetItems()
		...
		It("should deploy a second release with updated image", func() {
			...
			created, rel1 := shared.CreateStackAndDeploy(client, orgID, projectName, stack)
			stackID := created.GetId()
			namespace := created.GetNamespace()
			...
			shared.WaitForReleaseReleased(client, orgID, projectName, stackID, rel1.GetId(), 5*time.Minute)
			shared.WaitForStackReady(client, orgID, projectName, stackID, 2*time.Minute)
			...
			rel2 := shared.CreateRelease(client, orgID, projectName, stackID)
			Expect(rel2.GetSequence()).To(Equal(int32(2)))

			By("Waiting for release #2 to converge")
			shared.WaitForReleaseReleased(client, orgID, projectName, stackID, rel2.GetId(), 5*time.Minute)
		...
		It("should rollback to a previous release", func() {
			...
			rel3 := shared.RollbackRelease(client, orgID, projectName, stackID, rel1.GetId())
			...
			shared.WaitForReleaseReleased(client, orgID, projectName, stackID, rel3.GetId(), 5*time.Minute)
		...
		It("should supersede a pending release with a new one", func() {
			...
			rel1 := shared.CreateRelease(client, orgID, projectName, stackID)
			...
			r1 := shared.GetRelease(client, orgID, projectName, stackID, rel1.GetId())
